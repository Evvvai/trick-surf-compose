...
public void route_checker(int client){ //Check contains tricks
	...
	for (int i = 0; i < size; i += 5) {
		g_tricks.GetString(i, _route, sizeof(_route));
		g_tricks.GetString(i+2, _trick_name, sizeof(_trick_name));
		g_tricks.GetString(i+4, _speed_trick, sizeof(_speed_trick));
...
public void complete_trick(int client, int trick_index){ //Called then trick complete
    ...
    g_tricks.GetString(trick_index + 1, trick_id, sizeof(trick_id));
