...
ArrayList 	g_hop_triggers;

// Map data
int 		g_map_id = 3;
char 		g_map_name[64] = "none";
...
Handle g_pre_bhop_speed		= INVALID_HANDLE;
...
public void OnPluginStart() {
	...
	// Convars
	g_hop  				= CreateConVar("gxd_hop", 			"0", 	"Enable/Disable allow jumps | default 0");
	g_pre_speed 		= CreateConVar("gxd_pre_speed", 	"277", 	"Ammount pre strafe speed | default 405");
...
public void OnTriggerEndTouch(const char[] output, int caller, int activator, float delay) {
	...
	if (!is_valid_client(client) || !IsPlayerAlive(client) || g_map_id == -1)
		return;
...
public Action PlayerJumpEvent(Event event, const char[] name, bool dontBroadcast) {
	...
	int size = g_hop_triggers.Length;
	char _name[64];

	for (int i = 0; i < size; i += 1) {
		g_hop_triggers.GetString(i, _name, sizeof(_name));
